 ...
 */
std::vector<NrSlSciF1aHeader::SciStage2Format_t> NrSlSciF1aHeader::m_allowedSciStage2Format = {
    SciFormat2A};
...
void
NrSlSciF1aHeader::SetReselCounter(uint8_t reselCounter)
{
    m_reselCounter = reselCounter;
}
...
NrSlSciF1aHeader::SetSlMaxNumPerReserve(uint8_t slMaxNumPerReserve)
{
    ...
    valueCheck =
        (slMaxNumPerReserve == 1) || (slMaxNumPerReserve == 2) || (slMaxNumPerReserve == 3);
    NS_ASSERT_MSG(valueCheck,
                  "Invalid value " << +slMaxNumPerReserve
                                   << " for SlMaxNumPerReserve. Only 1, 2, or 3 should be used");
...
{
    NS_ASSERT_MSG(m_slMaxNumPerReserve == 2 || m_slMaxNumPerReserve == 3,
                  "SlMaxNumPerReserve should be set to 2 or 3 before setting GapReTx1");
    m_gapReTx1 = gapReTx1;
}
...
{
    NS_ASSERT_MSG(m_slMaxNumPerReserve == 3,
                  "SlMaxNumPerReserve should be set to 3 before setting GapReTx2");
    NS_ASSERT_MSG(gapReTx2 != GetGapReTx1(),
                  "The second retransmission should be perform in a different slot than the first "
                  "retransmission");
    m_gapReTx2 = gapReTx2;
}
...
NrSlSciF1aHeader::SetSciStage2Format(uint8_t formatValue)
{
    ...
    NS_ASSERT(std::find(m_allowedSciStage2Format.begin(),
                        m_allowedSciStage2Format.end(),
                        formatValue) != m_allowedSciStage2Format.end());
...
NrSlSciF1aHeader::SetIndexStartSbChReTx1(uint8_t sbChIndexReTx1)
{
    NS_LOG_FUNCTION(this);
    NS_ASSERT_MSG(m_slMaxNumPerReserve == 2 || m_slMaxNumPerReserve == 3,
                  "SlMaxNumPerReserve should be set to 2 or 3 before calling this method");
...
{
    NS_LOG_FUNCTION(this);
    os << "Priority " << +m_priority << ", MCS " << +m_mcs << ", 2nd stage SCI format "
       << +m_slSciStage2Format << ", Resource reservation period " << +m_slResourceReservePeriod
       << ", Total number of Subchannels " << +m_totalSubChannels << ", Index starting Subchannel "
       << +m_indexStartSubChannel << ", Total number of allocated Subchannel "
       << +m_lengthSubChannel << ", Maximum number of reservations " << +m_slMaxNumPerReserve
       ...
       << ", Reselection Counter " << +m_reselCounter;
}
...
NrSlSciF1aHeader::GetSerializedSize() const
{
    ...
    totalSize =
        (m_slMaxNumPerReserve == 3
             ? totalSize + 4
             : totalSize +
                   0); // all gapReTx1, indexStartSbChReTx1, gapReTx2, and indexStartSbChReTx2
...
NrSlSciF1aHeader::operator==(const NrSlSciF1aHeader& b) const
{
    return m_priority == b.m_priority && m_mcs == b.m_mcs &&
           ...
           m_indexStartSbChReTx2 == b.m_indexStartSbChReTx2 && m_reselCounter == b.m_reselCounter;
}
