...
class NrSlSciF1aHeader : public Header
{
  ...
  private:
    ...
    uint16_t m_totalSubChannels{
        std::numeric_limits<uint16_t>::max()}; //!< The total number of sub-channels
    // SCI fields
    ...
    uint16_t m_slResourceReservePeriod{
        std::numeric_limits<uint16_t>::max()}; //!< Resource reservation period
    uint8_t m_slMaxNumPerReserve{
        std::numeric_limits<uint8_t>::max()}; //!< maximum number of reserved resources
    uint8_t m_slSciStage2Format{
        std::numeric_limits<uint8_t>::max()}; //!< maximum number of reserved resources
    // SCI fields end
    ...
    uint8_t m_indexStartSbChReTx2{
        std::numeric_limits<uint8_t>::max()}; //!< The index of the starting sub-channel allocated
                                              //!< to second retransmission
    uint8_t m_gapReTx1{std::numeric_limits<uint8_t>::max()}; //!< The gap between a transmission and
                                                             //!< its first retransmission in slots
    uint8_t m_gapReTx2{std::numeric_limits<uint8_t>::max()}; //!< The gap between a transmission and
                                                             //!< its second retransmission in slots
    static std::vector<SciStage2Format_t>
        m_allowedSciStage2Format; //!< Vector of allowed Stage 2 SCI formats, to speed up checking
};
