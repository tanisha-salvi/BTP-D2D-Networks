    ...
    case IDLE: {
             ...
             it++, rbI++)
        {
            if (*it != 0)
            {
                ...
                // std::cout << (int)params->nodeId << ", " << (int)m_rnti << " : " << rbI << std::endl;
                rbMap.push_back(rbI);
            }
        }
...
NrSpectrumPhy::RxSlPscch(std::vector<uint32_t> paramIndexes)
{
    ...
    for (auto& ctrlMsgIt : sortedControlMessages)
    {
        ...
        // std::cout << "RxSlPscch " << (int)traceParams.m_dstL2Id << " " << (int)traceParams.m_rnti << std::endl;
    }
    ...
    for (auto& tbIt : m_slTransportBlocks)
    {
        ...
        if (m_slDataErrorModelEnabled)
        {
            ...
            if (sciF2aCorrupted)
            {
                ...
                // std::cout <<  "RxSlPssch " << (int)traceParams.m_dstL2Id << " " << (int)traceParams.m_srcL2Id
                        //   << std::endl;
