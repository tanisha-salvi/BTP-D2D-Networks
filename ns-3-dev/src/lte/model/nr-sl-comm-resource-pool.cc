...
NrSlCommResourcePool::SetNrSlPreConfigFreqInfoList(
    const std::array<LteRrcSap::SlFreqConfigCommonNr, MAX_NUM_OF_FREQ_SL>& slPreconfigFreqInfoList)
{
    NS_LOG_FUNCTION(this);
...
NrSlCommResourcePool::operator==(const NrSlCommResourcePool& other) const
{
    ...
    else
    {
        ...
        std::array<LteRrcSap::SlResourcePoolConfigNr, MAX_NUM_OF_TX_POOL> slTxPoolLocal =
...
NrSlCommResourcePool::GetSlResourcePoolNr(uint8_t bwpId, uint16_t poolId) const
{
    ...
    std::array<LteRrcSap::SlResourcePoolConfigNr, MAX_NUM_OF_TX_POOL> slTxPoolSelectedNormal =
                                               ...
                                               uint16_t t2) const
{
    ...
    uint16_t totalSlSymbols =
        LteRrcSap::GetSlLengthSymbolsValue(slBwpConfigCommon.slBwpGeneric.slLengthSymbols);
    uint16_t slSymbolStart =
        LteRrcSap::GetSlStartSymbolValue(slBwpConfigCommon.slBwpGeneric.slStartSymbol);
    ...
    t2min = 10;
    // std::cout << "t2min = " << t2min << " " << multiplier << "\n";
    ...
    for (uint64_t i = firstAbsSlotIndex; i <= lastAbsSlotIndex; ++i)
    {
        if (phyPool[absPoolIndex] == 1) // slot is a sidelink slot
        {
            ...
            uint16_t numSlPscchRbs =
                LteRrcSap::GetSlFResoPscchValue(pool.slPscchConfig.slFreqResourcePscch);
            uint16_t slPscchSymStart = slSymbolStart;
            uint16_t slPscchSymLength =
                LteRrcSap::GetSlTResoPscchValue(pool.slPscchConfig.slTimeResourcePscch);
            // PSSCH
...
NrSlCommResourcePool::GetNrSlSensWindInSlots(uint8_t bwpId, uint16_t poolId, Time slotLength) const
{
    ...
    LteRrcSap::SlSensingWindow slSensingWindowLen =
        slResourcePoolConfig.slResourcePool.slUeSelectedConfigRp.slSensingWindow;
    NS_ASSERT_MSG(slSensingWindowLen.windSize != LteRrcSap::SlSensingWindow::INVALID,
...
NrSlCommResourcePool::GetNrSlSubChSize(uint8_t bwpId, uint16_t poolId) const
{
    ...
    NS_ASSERT_MSG(slSubChSize.numPrbs != LteRrcSap::SlSubchannelSize::INVALID,
